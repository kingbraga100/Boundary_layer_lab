...
# =========================
FILE_PATH = r"C:\Users\ricar\OneDrive\Desktop\VKI\ELab - Experimental Laboratory Exercise\2025_BL\BL_GroupD_2025\data\Calibration\day3_Setra_Calibration_V3.csv"
...
FILE_PATH = DATA / "Calibration" / "day3_Setra_Calibration_V3.csv"
...
print(f"Detected {len(regions)} stable plateaus (after cleanup)")
print(f"Thresholds -> MAD ≤ {mad_thresh:.6f} V, |slope| ≤ {slope_thresh:.6f} V/s")

if len(regions) >= target:
    regions  = regions[:target]
    averages = averages[:target]
    pressures = np.arange(0, 11)
else:
    pressures = np.arange(0, len(regions))

# =========================
# Linear regression: P = a*V + b
# =========================
if len(averages) >= 2:
    a, b = np.polyfit(averages, pressures, 1)
    pred = a * averages + b
    ss_res = np.sum((pressures - pred) ** 2)
    ss_tot = np.sum((pressures - np.mean(pressures)) ** 2)
    r2 = 1 - ss_res / ss_tot if ss_tot > 0 else 1.0
    print(f"Linear regression: Pressure = {a:.6f} * Voltage + {b:.6f}")
...
# ---- Pressure vs Voltage with Regression + Text ----
plt.figure(figsize=(6,5))
plt.scatter(averages, pressures, label='Data points')
if len(averages) >= 2:
    idx = np.argsort(averages)
    plt.plot(averages[idx], pred[idx], 'r-', label='Linear fit')

    # Add regression info text box on the plot
    eq_text = f"Pressure = {a:.3f} × Voltage + {b:.3f}\n$R^2$ = {r2:.4f}"
...
plt.title('Pressure vs Voltage (0 → 10)')
plt.legend()
plt.grid(True)
